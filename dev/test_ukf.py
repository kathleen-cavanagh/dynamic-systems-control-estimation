...
from estimation.unscented_kf import UnscentedKF
...
P0 = np.eye(4)
Q = .01 * np.eye(cp.n_state)
R = np.eye(2)
k = 1

ukf = UnscentedKF(cp, x0, P0)
...
for sys_info in simulated_sys[1:]:
    ...
    ukf.propagate(policy(sys_info['t'], ukf.x), sys_info['t'], Q, k)
    ...
    ukf.update(
        meas, measurement_method, np.diag([.05, .05]), k)

    state_estimate.append(ukf.x)
